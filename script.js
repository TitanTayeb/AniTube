...
window.onload = function () {
    ...
    function createCarouselContainer(animeData) {
        ...
        watchNowButton.innerHTML = '<i class="fi fi-ss-play-circle"></i><p>Watch Now</p>';
...
function displaySearchResults(searchText, matchingAnimes) {
    ...
    searchLink.innerHTML = `<p>Search for : <b>${searchText}</b> </p><i class="fi fi-ss-arrow-circle-right"></i>`;
...
document.addEventListener("DOMContentLoaded", () => {
    const viewButtons = document.querySelectorAll(".view_type span");
    const container = document.querySelector(".container");

    // Load the saved view type from localStorage or default to 'three-view'
    const savedView = localStorage.getItem("selectedView") || "three";
    container.classList.add(`${savedView}-view`);
    document.querySelector(`.view_type .view_${savedView}`).classList.add("active");
    ...
    viewButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            // Save the selected view type to localStorage
...
function applyTheme(isLight) {
    if (isLight) {
        themeSwitch.classList.add('active'); // Add active class to the toggle
        document.body.classList.add('light'); // Add light class to the body
    } else {
        themeSwitch.classList.remove('active'); // Remove active class
        document.body.classList.remove('light'); // Remove light class from the body
    }
}

// Event listener for the theme toggle
themeSwitch.addEventListener('click', () => {
    const isLight = !themeSwitch.classList.contains('active'); // Toggle the state
    applyTheme(isLight);
