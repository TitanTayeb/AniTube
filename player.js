document.addEventListener('DOMContentLoaded', () => {
    ...
    if (anime) {
        ...
        langButtons.japanese.style.background = '#2D3142';
        animeNameElem.innerHTML = `<b>Now Playing:</b> ${anime.name}`;
    ...
    langButtons.english.addEventListener('click', () => {
        if (anime.english) {
            ...
            resetButtonStyles();
            langButtons.english.style.background = '#2D3142';
    ...
    langButtons.hindi.addEventListener('click', () => {
        if (anime.hindi) {
            ...
            resetButtonStyles();
            langButtons.hindi.style.background = '#2D3142';
...
document.addEventListener('DOMContentLoaded', () => {
    function getQueryParams() {
        const params = new URLSearchParams(window.location.search);
        return {
            name: params.get('name'), // Get the anime name from the URL query
        };
    }

    const { name } = getQueryParams();

    // Assuming animes data is available
    const anime = animes.find(anime => anime.name === name);
    ...
    if (anime) {
        ...
        // Update the page with anime details
        if (cover && banner && newTitle && rating && wiki) {
            cover.style.backgroundImage = `url(${anime.img})`;
            cover.style.backgroundSize = 'cover';
            cover.style.backgroundPosition = 'center';
            banner.src = anime.img;
            newTitle.innerHTML = anime.name;
            rating.innerHTML = `${anime.rating}`;
            wiki.href = anime.wiki;  // Set wiki link
            wiki.innerHTML = `See more at: <b>${anime.wiki}</b>`;  // Set link text
        }
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (anime && anime.plot) {
        const plotSpan = document.getElementById('plot');
        ...
        fetch(anime.plot)
            ...
            .then(data => {
                // Truncate the plot text if it's too long and show "View More"
                const maxLength = 200;
                if (data.length > maxLength) {
                    plotSpan.textContent = data.slice(0, maxLength) + '...';
                    viewMoreButton.style.display = 'inline-block';
                } else {
                    plotSpan.textContent = data;
                    viewMoreButton.style.display = 'none';
                }

                // Show full plot when "View More" is clicked
            ...
            .catch(error => {
                plotSpan.textContent = 'Error loading plot: ' + error.message;
            });
    }
});
...
window.addEventListener('load', adjustSpacing);
    ...
    function adjustSpacing() {
      ...
      animeInfo.style.marginTop = headerHeight + '55px';
